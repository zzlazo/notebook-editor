...
- **レスポンシブ対応**: 画面が横長なら仕様どおりのレイアウト、縦長ならサイドバーを Drawer に収める
  - 仕様は PC 横長のみのため、縦長では余白とレイアウトを調整するアレンジを行った
    ...
    - Drawer がステータスバーとジェスチャーバーに重ならないようにした
- **選択中のページを URL に持たせる**（`/contents/:id`）: リロードしても選択が保たれ、戻る操作で前に開いていたページに戻れる。存在しないページの URL を開いた場合はページの選択を促す
...
- サーバー由来のデータは Provider、選択中のページは URL、編集モードかどうかは画面ローカルな状態（hooks）に置いています
...
| `AppButton` | `onPressed` が `null` のとき disabled になり不透明度で表現されること。高さ 40px と幅（regular / mini）、アイコン 24px 四方 | component は Provider に依存せず、値を渡すだけで単体でテストできる（[状態の置き場所とコンポーネント分割](#状態の置き場所とコンポーネント分割)） |
...
- 編集モードを持つカスタムフック（`useContentEditingState`）はテストしていません。`useState` を包んで操作に名前を付けただけで、確かめられるのが flutter_hooks 自体の挙動になるためです
